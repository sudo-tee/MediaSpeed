...
const showFields = ["backdrop_path", "created_by", "first_air_date", "genres", "homepage", "in_production", "languages", "last_air_date", "name", "networks", "number_of_episodes", "number_of_seasons", "origin_country", "original_language", "original_name", "overview", "popularity", "poster_path", "production_companies", "status", "vote_average", "vote_count"];

const seasonFields = ["air_date", "name", "overview", "poster_path", "season_number"];

const episodeFields = ["episode_number", "season_number", "name", "air_date", "crew", "guest_stars", "overview", "id", "production_code", "still_path", "vote_average", "vote_count"];
...
export default class EpisodeTmdbInfoProvider {
    ...
    async getepisodeInfo(episode) {
        ...
        return {
            show: {...episode.show, ...pick(showDetails, showFields), ...{tmdb_id: showDetails.id}},
            episode: {...episode.episode, ...pick(episodeDetails, episodeFields), ...{tmdb_id: episodeDetails.id}},
            season: {...episode.season, ...pick(seasonDetails, seasonFields), ...{tmdb_id: seasonDetails.id}}
        };
    ...
    async getEpisodeDetails(showId, season) {
        ...
        return this.movieDbApi.request('/tv/{id}?append_to_response={append_to_response}', 'GET', {id: encodeURIComponent(showId), append_to_response: "season/" + season});
    ...
    getSeasonDetails(data, seasonNumber) {
        ...
        if(data['season/' + seasonNumber])  {
            seasonData = {...data['season/' + seasonNumber], ...data['seasons'][seasonNumber]}
        }
        return seasonData;
    }
}
