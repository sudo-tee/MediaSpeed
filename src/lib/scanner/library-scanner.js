...
export default class LibraryScanner {
    constructor(directoryScanner, movieInfoProvider, episodeInfoProvider, movieService, libraryService, episodeService,  logger) {
    ...
    async scanMovies(library) {
        ...
        return Promise.all(newMoviesFiles.map(async (path) => this.movieService.create(await this.movieInfoProvider.execute(path, library))));
    }

    async scanEpisodes(library) {
        const newEpisodeFiles = await this.directoryScanner.getNewFiles(library.path);

        return Promise.all(newEpisodeFiles.map(async (path) => this.createEpisode(await this.episodeInfoProvider.execute(path, library))));
    }
