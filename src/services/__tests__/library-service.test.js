...
describe('LibraryService', () => {
    ...
    describe('create', () => {
        ...
        it('removes unknown props', async () => {
            const { service } = setup();
            expect(await service.create({ title: 'test', removeme: 'please' })).toEqual({
                title: 'test'
            });
        });
    });
    ...
    describe('update', () => {
        it('throws when todo does not exist', async () => {
            const { service } = setup();
            expect((await throws(service.update('nonexisting', {}))).message).toMatch(/nonexisting/);
    ...
    describe('remove', () => {
        it('throws when todo does not exist', async () => {
            const { service } = setup();
            expect((await throws(service.remove('nonexisting'))).message).toMatch(/nonexisting/);
...
function setup() {
    const todos = [{ id: '1', title: 'Todo 1', completed: true }, { id: '2', title: 'Todo 2', completed: false }];
    // Mock store
    const store = {
    ...
    return { service: new LibraryService(store), store, todos };
}
